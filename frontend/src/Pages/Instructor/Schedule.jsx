...
export default function InstructorSchedule() {
  ...
  const [formData, setFormData] = useState({ day: '', time: '', course: '', location: '' });
  ...
  const handleCancelLecture = (index) => {
    ...
    setNotification(`Lecture on ${canceled.day} for ${canceled.course} has been canceled. Students notified.`);
    setTimeout(() => setNotification(''), 5000);
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-100 p-4 sm:p-6">
      <h1 className="text-2xl sm:text-3xl font-bold text-indigo-800 mb-4 sm:mb-6">Instructor Schedule</h1>
          ...
          <tbody>
            {schedule.map((session, index) => (
              <tr key={index} className="hover:bg-indigo-100 transition">
                <td className="px-2 sm:px-4 py-2 font-medium text-indigo-700">{session.day}</td>
                <td className="px-2 sm:px-4 py-2 text-gray-700">{session.time}</td>
                <td className="px-2 sm:px-4 py-2 text-gray-700">{session.course}</td>
