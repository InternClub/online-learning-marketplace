...
const systemSettingsSchema = new mongoose.Schema({
  ...
  category: {
    ...
    required: true,
    enum: [
      'general', 'payment', 'email', 'security', 'courses', 'users', 
      'notifications', 'analytics', 'appearance', 'integrations',
      'content_management', 'instructor_settings', 'learner_settings'
    ]
  },
...
systemSettingsSchema.virtual('isValid').get(function() {
  ...
  if (this.dataType === 'string') {
    if (validation.minLength !== undefined && val.length < validation.minLength) return false;
    if (validation.maxLength !== undefined && val.length > validation.maxLength) return false;
    if (validation.pattern && !new RegExp(validation.pattern).test(val)) return false;
  }
...
systemSettingsSchema.methods.updateValue = function(newValue, changedBy, reason = '') {
...
systemSettingsSchema.methods.rollback = function(changedBy, reason = 'Rollback') {
...
systemSettingsSchema.statics.getByCategory = function(category, includeSecrets = false) {
